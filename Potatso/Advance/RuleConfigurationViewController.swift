...
class RuleConfigurationViewController: FormViewController {
    ...
    init(rule: Rule?, callback: ((Rule) -> Void)?) {
        if let rule = rule {
            ...
            self.rule = Rule(type: RuleTypes.DomainSuffix, action: RuleAction.Proxy, value: "")
            isEdit = false
        }
    ...
    func generateForm() {
        form +++ Section()
            <<< PushRow<RuleTypes>(kRuleFormType) {
                $0.title = "Type".localized()
                $0.selectorTitle = "Choose type of rule".localized()
                $0.options = [RuleTypes.DomainSuffix, RuleTypes.DomainMatch, RuleTypes.Domain, RuleTypes.IPCIDR, RuleTypes.GeoIP]
    ...
    func save() {
        do {
            ...
            guard let type = values[kRuleFormType] as? RuleTypes else {
